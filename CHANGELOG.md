...
### Added
- **"Show Descriptions" toggle (showcase level).** New checkbox in Layout & Structure that hides or shows item descriptions for the whole showcase without deleting the text. Lets you run some showcases title-only and others with descriptions, and flip back anytime. Defaults ON, so existing showcases are unchanged on upgrade. Combined with the v2.1.1 empty-description fix, cards stay clean whether descriptions are off globally or just blank on a given item.
...
### Fixed
- **Empty description no longer leaves a gap.** The description `<p>` was rendered unconditionally, so cards with a blank description showed an empty paragraph (with its margin), leaving a small dead gap between the title and "Read more." The paragraph is now only output when the item has description text — blanking a description in admin yields a clean title-only card. Per-card; cards that have text are unchanged.
